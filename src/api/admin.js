...
export async function searchUsers({ query = "", limit = 50 } = {}) {
  let req = supabase
    .from("profiles")
    .select("id, username, first_name, last_name, email, phone, avatar_url, is_blocked, is_admin, reputation")
    .order("username", { ascending: true })
    .limit(limit);

  if (query.trim()) {
    const q = `%${query.trim()}%`;
    req = req.or(`username.ilike.${q},email.ilike.${q},first_name.ilike.${q},last_name.ilike.${q}`);
  }
...
export async function getAllPosts({ sortBy = "newest", search = "", limit = 50 } = {}) {
  let req = supabase
    .from("posts")
    .select(`
  ...
  if (sortBy === "oldest") {
    ...
    req = req.order("score", { ascending: false }).order("created_at", { ascending: false });
  } else if (sortBy === "comments") {
    req = req.order("comment_count", { ascending: false }).order("created_at", { ascending: false });
  } else {
